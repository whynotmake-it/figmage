...
- `inheritance`: A list of interfaces for specific collections to implement. See the [Implementing Interfaces](/reference/implementing-interfaces) documentation for more information.
...
Figma allows you to delete variables that are still referenced by other variables or bound to properties in your design. 
These variables are marked as "deleted but referenced" in the Figma API.

By default (`false`), Figmage excludes these deleted variables from token generation and logs a warning message. 
This ensures your generated tokens only include active variables that are still maintained in your design system.

If you set this flag to `true`, deleted but referenced variables will be included in the generated tokens.
This might be useful if you have a complex design system with dependencies that you want to preserve temporarily.

:::tip[Recommended Approach]
It's generally recommended to keep this setting as `false` (default) and instead:
1. Review the warning messages to identify deleted but referenced variables
2. Either restore the deleted variables in Figma, or remove the references to them
3. This helps maintain a clean and consistent design system
:::
