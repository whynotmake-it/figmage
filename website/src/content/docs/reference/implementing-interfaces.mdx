...
In your configuration, use the `inheritance` key under a token type (e.g., `colors`, `typography`). Each entry in `inheritance` can specify which collections it applies to and a list of interfaces to implement.
